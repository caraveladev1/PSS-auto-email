...
async function getDataSampleStatus() {
  try {
    ...
    const sqlDataSC = 'SELECT * FROM listSampleStateRH';
...
async function updateEmailStatus(sample_id) {
  try {
    const pool = await connectDB();
    const sqlUpdateEmailStatus = 'UPDATE listSampleStateRH SET [email_sent] = 2 WHERE [sample_id] = @sample_id';
...
async function enviarCorreoAccepted(dataOrganizada) {
  try {
    //console.log(dataOrganizada)
    const sample_id_NewPending = dataOrganizada.filter(sample_id => sample_id.email_sent === '1' && sample_id.customer_sample_feedback === 'Approved');
    ...
    if (sample_id_NewPending.length > 0) {
      for (const sample_id of sample_id_NewPending) {
        const mailOptions = {
          ...
          text: `The following Sample has change the state in RoasterHub: 

          Sample id: ${sample_id.sample_id}
          Contract: ${sample_id.Contract}
        ...
        console.log(pc.yellow(`[SAMPLE STATE RH]: Correo enviado ${sample_id.sample_id}`));
      }
    } else { console.log(pc.yellow('[SAMPLE STATE RH]: No hay correos con estado "accepted" por enviar')); }
  } catch (error) {
...
async function enviarCorreoRejected(dataOrganizada) {
  try {
    ...
    } else { console.log(pc.yellow('[SAMPLE STATE RH]: No hay correos con estado "Rejected" por enviar')); }
  } catch (error) {
    console.error('Error al enviar correos electrónicos:', error);
    throw error;
  }
...
export async function startSampleStateRHAutomatization() {
