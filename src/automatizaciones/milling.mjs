...
async function enviarCorreo(cliente) {
  try {
    ...
    const muestrasPendientes = cliente.sampleData.filter((sample) => sample.mail_sent === '1');
...
`,
      ...
      console.log(pc.cyan('[MUESTRAS MILLING]'), `Successfully Notification email sent to ${cliente.customer}`);
      ...
      for (const muestra of muestrasPendientes) {
        ...
        console.log(pc.cyan('[MUESTRAS MILLING]'), `Updating Notification email status for Sample ID: ${sampleIdToUpdate}`);
